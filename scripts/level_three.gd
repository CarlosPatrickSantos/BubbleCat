...
@onready var enemy_fada_spawner = $EnemyFadaSpawner # <--- Adicione esta linha
...
func _on_bird_scored():
	...
	if GameManager.score == LEVEL_TWO_SCORE:
		...
		if is_instance_valid(enemy_fada_spawner):
			enemy_fada_spawner.can_spawn = false # OU enemy_bird_spawner.stop_spawning() se você tiver essa função.
	# ===================================================
	# GERAÇÃO DO PORTAL (USANDO O SPANWER DA FASE 2)
	# ===================================================
	if GameManager.score == LEVEL_TWO_SCORE:
...
func _on_portal_entered():
	...
	#get_tree().change_scene_to_file("res://scene/game.tscn")
	get_tree().change_scene_to_file.call_deferred("res://scene/level_four.tscn")

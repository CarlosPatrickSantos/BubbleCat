...
@onready var enemy_bird_spawner = $EnemyBirdSpawner # <--- Adicione esta linha
...
func _on_bird_scored():
	...
	if GameManager.score == LEVEL_TWO_SCORE:
		...
		if is_instance_valid(enemy_bird_spawner):
			enemy_bird_spawner.can_spawn = false # OU enemy_bird_spawner.stop_spawning() se você tiver essa função.
	# ===================================================
	# GERAÇÃO DO PORTAL AOS 50 PONTOS (CORREÇÃO DE INDENTAÇÃO AQUI)
	# ===================================================
	if current_score == LEVEL_TWO_SCORE:
		call_deferred("spawn_portal") # <--- CORRIGIDO
		# Para de gerar CANOS para dar tempo do jogador ver o portal
		#pipe_spawner.stop_spawning() 
		# Opcional: Adicione um som de "ding" ou "Level Up" aqui!
...
func _on_portal_entered():
	...
	#get_tree().change_scene_to_file("res://scene/game.tscn")
	get_tree().change_scene_to_file.call_deferred("res://scene/level_three.tscn")
